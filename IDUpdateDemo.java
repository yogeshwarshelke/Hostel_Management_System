...
class IDUpdate implements ActionListener
{
	JFrame f;
	...
	public IDUpdate()
	{
		...
		l.setBounds(320,30,250,40);
		f.add(l);
		
		t=new JTextField(10);
		t.setFont(f3);
		t.setBounds(550,30,150,40);
		f.add(t);
		
		b=new JButton("Submit");
		b.setFont(f1);
		b.setBounds(710,30,150,40);
		f.add(b);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==b)
		{
			int idno=Integer.parseInt(t.getText());
			int flag=0;
			String gender="Male";
			...
			try
			{
				...
				String sql="select * from StudentAddmission where Idno=?";	
		  	   // sql="update StudentAddmission set Idno=? Roomno=? Date=? StudentName=? FatherName=? MotherName=? Address=? Tal=? Dist=? StudentCast=? SubCast=? Gender=? BirthDate=? NewClass=? PreviousCollege=? Percentage=? HF=? Af=? AR=? AC=? MobNo=? where Idno=?";
